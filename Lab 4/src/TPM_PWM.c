...
void configureTPM0forPWM() {
    ...
    // Disable the TPM0
    TPM0->SC = TPM_SC_CMOD(0) ;  // determines the clock source for the TPM. 
    ...
    TPM0->CONTROLS[TPM0_CHAN1].CnSC = 0x28 ;  // calculated based on 16 bit format 40 in decimal.
		// 101000
...
void setLEDBrightness(enum LED led, unsigned int brightness) {
    ...
    switch (led) {
        case Red:
            TPM2->CONTROLS[TPM2_CHAN1].CnV = TPM_CnV_VAL(d) ;  // This CnV value, sets the period of time the light is on for.
            break ;
        case Green:
